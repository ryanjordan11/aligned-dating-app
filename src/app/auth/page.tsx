...
export default function AuthPage() {
  ...
  // Source of truth is the URL so it can't desync during hydration.
  ...
  useEffect(() => {
    ...
    // Avoid redirect races while a submit is in-flight.
    if (busy) return;
  ...
  const submitPassword = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Convex Auth can surface "InvalidSecret" for wrong password; don't show internals to users.
  ...
  const submitGoogle = async () => {
    ...
    setError(null);
    try {
      // Send new accounts straight to onboarding; existing accounts can be redirected out by the /app layout guard.
